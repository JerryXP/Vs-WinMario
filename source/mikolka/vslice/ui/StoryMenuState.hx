...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var grid:FlxBackdrop = new FlxBackdrop(FlxGridOverlay.createGrid(80, 80, 160, 160, true, 0x33AA00FF, 0x0));
		...
		#end
		scoreText.setFormat(Paths.font("cool.ttf"), 32);

		txtWeekTitle = new FlxText(FlxG.width * 0.7, 10, 0, "", 32);
		txtWeekTitle.setFormat(Paths.font("week.ttf"), 32, FlxColor.WHITE, RIGHT);
		txtWeekTitle.alpha = 0.7;

		var ui_tex = Paths.getSparrowAtlas('campaign_menu_UI_assets');
		var bgYellow:FlxSprite = new FlxSprite(0, 56).makeGraphic(FlxG.width, 386, 0xF000000);
		...
		var tracksSprite:FlxSprite = new FlxSprite(FlxG.width * 0.05 + 130, bgSprite.y + 425).loadGraphic(Paths.image('Menu_Tracks'));
		...
		txtTracklist.font = Paths.font("track.ttf");
		txtTracklist.color = 0xFFF00FF;
		add(txtTracklist);
		add(scoreText);
		add(txtWeekTitle);
