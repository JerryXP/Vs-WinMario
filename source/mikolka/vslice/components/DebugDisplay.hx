...
class FunkinDebugDisplay extends Sprite
{
	...
	function createSimpleElements():Void
	{
		...
		infoDisplay.defaultTextFormat = new TextFormat('ModernDOS8x16.ttf', 14, color, JUSTIFY);
	...
	function updateAdvancedDisplay():Void
	{
		...
		#if windows
		info.push('OS: Windows');
		#elseif mac
		info.push('OS: macOS');
		#elseif linux
		info.push('OS: Linux');
		#elseif android
		info.push('OS: Android');
		#elseif ios
		info.push('OS: iOS (iPhone)');
		#elseif html5
		info.push('OS: HTML5');
		#else
		info.push('OS: Unknown');
		#end
