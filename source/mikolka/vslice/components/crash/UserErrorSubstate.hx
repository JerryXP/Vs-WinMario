...
class UserErrorSubstate extends MusicBeatSubstate
{
	...
	{
		FlxG.sound.play(Paths.sound('crash'), 0.7);
		super.create();
	...
	function printError(error:CrashData)
	{
		...
		printToTrace('LUNAR ENGINE ${MainMenuState.lunarVersion}$star  (${error.message})');
		textNextY += 35;
		FlxTimer.wait(1 / 24, () ->
		{
			printSpaceToTrace();
			...
			printToTrace('MOD:${error.activeMod.rpad(" ", 10)} PE:${MainMenuState.psychEngineVersion.rpad(" ", 5)} PS:${MainMenuState.pSliceVersion.rpad(" ", 5)} SYS:${error.systemName}');
			printSpaceToTrace();
            printToTrace(getLogger());
			if (isCritical)
				printToTrace('REPORT TO THE DEVS');
			else
				printToTrace('');
			if (isCritical)
			{
	...
	function printToTrace(text:String):FlxText
	{
		...
		test_text.setFormat(Paths.font('time.ttf'), 16, FlxColor.WHITE, LEFT);
