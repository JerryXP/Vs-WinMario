...
class CrashState extends FlxState
{
	...
	override function create()
	{
        ...
        static function saveError(error:mikolka.vslice.components.crash.UserErrorSubstate.CrashData)
        {
			...
			errMsg += 'Lunar Engine ${MainMenuState.lunarVersion}$star\n';
			errMsg += '\n';
            errMsg += 'P-Slice ${MainMenuState.pSliceVersion}$star\n';
			errMsg += 'Psych Engine ${MainMenuState.psychEngineVersion}$star\n';
			errMsg += 'WinMario ${MainMenuState.winmarioVersion}$star\n';
            ...
            errMsg += '\n';
            errMsg += '\nUH OH! SOMETHING WENT WRONG, PLEASE RESTART THE APP\n
			IF THIS ERROR CONTINUES PLEASE REPORT IT TO THE DEVS. THANK YOU.\n\n> Crash Handler written by: sqirra-rng';
    
            var path = './crash/' + 'LunarEngine_' + dateNow + '.txt';
