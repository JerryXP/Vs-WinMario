...
class PauseSubState extends MusicBeatSubstate
{
	...
	override function create()
	{
		...
		var grid:FlxBackdrop = new FlxBackdrop(FlxGridOverlay.createGrid(80, 80, 160, 160, true, 0x7FFFFFFF, 0x0));
		grid.velocity.set(40, 40);
		grid.alpha = 0;
		FlxTween.tween(grid, {alpha: 1}, 0.5, {ease: FlxEase.quadOut});
		add(grid);
		}
		FlxG.sound.play(Paths.sound('pause_sfx'), 0.7);
		...
		var ballsTxt = inVid ? Language.getPhrase("pause_branding", '{1} Paused',
			[cutscene_branding]) : Language.getPhrase("blueballed", "Deaths: {1}", [PlayState.deathCounter]);
	...
	{
		var formattedSongName:String = (songName != null ? "pause/" + Paths.formatToSongPath(songName) : '');
		var formattedPauseMusic:String = "pause/" + Paths.formatToSongPath(ClientPrefs.data.pauseMusic);
		...
		if (cantUnpause <= 0)
		{
			...
			switch (selectedOption)
			{
				...
				case 'Options':
					...
					{
						FlxG.sound.playMusic(Paths.music("pause/" + Paths.formatToSongPath(ClientPrefs.data.pauseMusic)), pauseMusic.volume);
