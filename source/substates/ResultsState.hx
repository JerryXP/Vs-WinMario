...
class ResultsState extends MusicBeatState
{
    ...
    override public function create()
    {
        ...
        var modOrGame = params.isMod && params.modFolder != null && params.modFolder != "" ? params.modFolder : "Vs. WinMario";
        var now = Date.now();
        ...
        flawlesss = new FlxText(leftX, judgY, 340, Language.getPhrase('judgement_flawlesss', 'Crazys') + ': 0', 32);
        flawlesss.setFormat(Paths.font("vcr.ttf"), 32, 0xFFFF00FF, "left");
        add(flawlesss);

        sicks = new FlxText(rightX, judgY, 340, Language.getPhrase('judgement_sicks', 'Cools') + ': 0', 32);
        sicks.setFormat(Paths.font("vcr.ttf"), 32, 0xFF00FFFF, "left");
        add(sicks);

        goods = new FlxText(leftX, judgY + judgSpacing, 340, Language.getPhrase('judgement_goods', 'Goods') + ': 0', 32);
        goods.setFormat(Paths.font("vcr.ttf"), 32, 0xFF00FF00, "left");
        add(goods);

        bads = new FlxText(rightX, judgY + judgSpacing, 340, Language.getPhrase('judgement_bads', 'Okays') + ': 0', 32);
        bads.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.YELLOW, "left");
        add(bads);

        shits = new FlxText(leftX, judgY + judgSpacing * 2, 340, Language.getPhrase('judgement_shits', 'Awfuls') + ': 0', 32);
        shits.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.RED, "left");
        add(shits);

        misses = new FlxText(rightX, judgY + judgSpacing * 2, 340, Language.getPhrase('judgement_misses', 'Nopes') + ': 0', 32);
        misses.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.PURPLE, "left");
        add(misses);

        comboText = new FlxText(leftX, judgY + judgSpacing * 3 - 14, 700, Language.getPhrase('judgement_max_combo', 'Highest Combo') + ': 0', 26);
        comboText.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, "left");
        add(comboText);

        accText = new FlxText(leftX, judgY + judgSpacing * 3 + 20, 700, Language.getPhrase('results_accuracy', 'Accuracy') + ': 0%', 26);
        accText.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, "left");
        add(accText);
        ...
        graphLabel.setFormat(Paths.font("vcr.ttf"), 18, FlxColor.WHITE, "center");
        add(graphLabel);
        ...
        if (params.isPractice != null && params.isPractice) {
            ...
            practiceText.setFormat(Paths.font("vcr.ttf"), 22, FlxColor.YELLOW, "center");
            add(practiceText);
            yBottom += 28;
        }

        var engineInfo = Language.getPhrase('winmario_version', 'WinMario v') + MainMenuState.winmarioVersion;
        var engineText = new FlxText(0, FlxG.height - 100, FlxG.width, engineInfo, 25);
        engineText.setFormat(Paths.font("vcr.ttf"), 25, FlxColor.CYAN, "center");
        add(engineText);
        ...
        #end
        continueText.setFormat(Paths.font("vcr.ttf"), 26, FlxColor.WHITE, "center");
        add(continueText);
    ...
    override public function update(elapsed:Float)
    {
        ...
        if (animatedFlawlesss < params.flawlesss) {
            ...
            flawlesss.text = Language.getPhrase('judgement_marvelouss', 'Crazy') + ': $animatedFlawlesss';
            return;
        }
        flawlesss.text = Language.getPhrase('judgement_marvelouss', 'Crazy') + ': $animatedFlawlesss';
        if (animatedSicks < params.sicks) {
            animatedSicks = animateInt(animatedSicks, params.sicks);
            sicks.text = Language.getPhrase('judgement_sicks', 'Cool') + ': $animatedSicks';
            return;
        }
        sicks.text = Language.getPhrase('judgement_sicks', 'Cool') + ': $animatedSicks';
        ...
        if (animatedGoods < params.goods) {
            ...
            goods.text = Language.getPhrase('judgement_goods', 'Good') + ': $animatedGoods';
            return;
        }
        goods.text = Language.getPhrase('judgement_goods', 'Good') + ': $animatedGoods';
        ...
        if (animatedBads < params.bads) {
            ...
            bads.text = Language.getPhrase('judgement_bads', 'Ok') + ': $animatedBads';
            return;
        }
        bads.text = Language.getPhrase('judgement_bads', 'Ok') + ': $animatedBads';
        ...
        if (animatedShits < params.shits) {
            ...
            shits.text = Language.getPhrase('judgement_shits', 'Awful') + ': $animatedShits';
            return;
        }
        shits.text = Language.getPhrase('judgement_shits', 'Awful') + ': $animatedShits';
        ...
        if (animatedMisses < params.misses) {
            ...
            misses.text = Language.getPhrase('judgement_misses', 'Nope') + ': $animatedMisses';
            return;
        }
        misses.text = Language.getPhrase('judgement_misses', 'Nope') + ': $animatedMisses';
