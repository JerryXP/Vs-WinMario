...
class OptionsState extends MusicBeatState
{
	...
	function openSelectedSubstate(label:String) {
		...
		switch(label)
		{
			...
			#if desktop
			case 'Desktop':
				openSubState(new DesktopSettingsSubState());
			case '\nGo back to the Desktop.':
				if (ClientPrefs.data.skipLogin != 'Both') {
				FlxG.sound.music.fadeOut();
				MusicBeatState.switchState(new states.StopandThink());
				} else {
					funnyCam.visible = persistentUpdate = true;
					UserErrorSubstate.makeMessage("Your Skipping Desktop Interface is both.", 
					"Please change your setting:
					\"Skip Interface\"\n
					We can't take you there if you 
					decided to skip both Login & 
					Desktop interfaces.");
				}
			#end
		}
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('optionsCenter'));
