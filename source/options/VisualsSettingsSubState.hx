...
{
	public static var pauseMusics:Array<String> = ['None', 'Buoy Base Galaxy', 'Rainbow Road (MK8)', 'Rainbow Road (DS)', 'Sunshine Airport', 'SNES Rainbow Road (MK8)', 'Death Egg Zone Act 1 (SnK Alt)', 'Death Egg Zone Act 2 (SnK Alt)', 'Steam Gardens (8-Bit)', 'DataLog (NEO R)', 'Tutorial (NEO R)(INST)', 'Bopeebo (NEO R)(INST)', 'South (NEO R)(INST)', 'Prelude (VIP Edit)', 'Rainbow Road (Wii)', 'Rogueport (Sublevel Three)', 'Another World', 'Bring Me Back to Live', 'C U Again', 'Slow', 'Infinite'];
	...
	public function new()
	{
		...
		var option:Option = new Option('Special Anim. on Crazy, Cool, or Awful.',
			'It will plays Alt Animations if you get Crazy/Cool\nAnd plays Miss Animation if you get Awful.\nTurn this off if you want to be boring.',
			'swagPlayAnimation',
			BOOL);
		addOption(option);
		...
		var option:Option = new Option('Time Bar:',
			...
			'timeBarType',
			STRING,
			['WinMario (simple)', 'WinMario (complex)', 'Time Elapsed', 'Time Left', 'Song Name', 'Disabled']);
		addOption(option);
		...
		var option:Option = new Option('ScoreTxt Info:',
			...
			'winmarioCustomscore',
			STRING,
			['WinMario', 'WinMario (Legacy)', 'Mic\'d Up', 'Codename', 'Yoshi', 'Psych', 'Kade', 'Vanillia', 'Forever', 'OS']);
		addOption(option);
		...
		var option:Option = new Option('Video Intro:',
			...
			'introvideoCutscene',
			STRING,
			['Normal', 'Alternate', 'WinXP', 'WinVista B1', 'Mario World', 'WM/LXP Mix', 'Legacy', 'Disabled']);
		addOption(option);
	...
	function onChangePauseMusic()
	{
		if(ClientPrefs.data.pauseMusic == 'None')
			...
			FlxG.sound.playMusic(Paths.music("pause/" + Paths.formatToSongPath(ClientPrefs.data.pauseMusic)));
