...
class InitState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (FlxG.save.data.flashing == null)
		{
			...
			#if mobile
			MusicBeatState.switchState(new FlashingState(new IntroVideoState()));
			#else
			MusicBeatState.switchState(new states.LoginStateFlash());
			#end
		}
        else if (mustUpdate && !updateAlreadyShown){
			updateAlreadyShown = true;
            MusicBeatState.switchState(new OutdatedState(updateVersion,new StopandThink()));
        }
		else
		{
			new FlxTimer().start(0.05, function(tmr:FlxTimer)
				{
					...
					#else
					MusicBeatState.switchState(new StopandThink());
					#end
				});
		}
	}
	...
	function fetchUpdateData()
	{
		if (ClientPrefs.data.checkForUpdates)
		{
			...
			var http = new haxe.Http("https://raw.githubusercontent.com/JerryXP/WinMario-Version-Checker/main/gitVersion.txt");

			http.onData = function(data:String)
			{
				updateVersion = data.split('\n')[0].trim();
				var curVersion:String = MainMenuState.winmarioVersion.trim();
