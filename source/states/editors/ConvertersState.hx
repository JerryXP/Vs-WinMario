...
class ConvertersState extends MusicBeatState
{
	...
	override function create()
	{
		...
		FlxG.sound.playMusic(Paths.music('converter_menu'));
		bg = new FlxSprite().loadGraphic(Paths.image('convertBG'));
		bg.scrollFactor.set();
		bg.color = 0xFF3F4F70;
		add(bg);

		bgText = new FlxSprite().loadGraphic(Paths.image('converter_text'));
	...
	function setPage(newPage:String, playSound:Bool = true)
	{
		...
		options = switch(page)
		{
			case PAGE_CHARTS: ['Godot Charts', 'Nightmare-Vision Charts', 'Codename Engine Charts', 'V-Slice Charts', 'Back'];
			case PAGE_CHARACTERS: ['Godot Characters', 'Codename Engine Characters', 'V-Slice Characters', 'Forever Engine Characters', 'Back'];
			case PAGE_STAGES: ['Codename Engine Stages', 'V-Slice Stages', 'Back'];
	...
	function getDescription(option:String):String
	{
		return switch(option)
		{
			...
			case 'V-Slice Charts': 'Open a V-Slice chart.json and metadata.json, then export chart JSON files.';
			...
			case 'V-Slice Stages': 'Convert one V-Slice stage JSON.';
	...
	static function parseGodotChart(raw:String, fileName:String):SwagSong
	{
		...
		songData.format = 'psych_v1_convert';
