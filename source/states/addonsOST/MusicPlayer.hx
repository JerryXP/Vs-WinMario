package states.addonsOST;

import flixel.group.FlxGroup;
import flixel.ui.FlxBar;
import flixel.util.FlxStringUtil;

import states.SoundtrackState;

/**
 * Music player used for Freeplay
 */
@:access(states.SoundtrackState)
class MusicPlayer extends FlxGroup 
{
	public var instance:SoundtrackState;
	...
	public function new(instance:SoundtrackState)
	{
		super();
	...
	override function update(elapsed:Float)
	{
		...
		var songName:String = instance.songs[SoundtrackState.curSelected].songName;
		...
		if (playing)
		{
			if(SoundtrackState.vocals != null)
				SoundtrackState.vocals.volume = (SoundtrackState.vocals.length > FlxG.sound.music.time) ? 0.8 : 0;
			if(SoundtrackState.opponentVocals != null)
				SoundtrackState.opponentVocals.volume = (SoundtrackState.opponentVocals.length > FlxG.sound.music.time) ? 0.8 : 0;

			if((SoundtrackState.vocals != null && SoundtrackState.vocals.length > FlxG.sound.music.time && Math.abs(FlxG.sound.music.time - SoundtrackState.vocals.time) >= 25) ||
			(SoundtrackState.opponentVocals != null && SoundtrackState.opponentVocals.length > FlxG.sound.music.time && Math.abs(FlxG.sound.music.time - SoundtrackState.opponentVocals.time) >= 25))
			{
				pauseOrResume();
	...
	{
		if (SoundtrackState.vocals != null && SoundtrackState.vocals.length > time)
			SoundtrackState.vocals.time = time;
		if (SoundtrackState.opponentVocals != null && SoundtrackState.opponentVocals.length > time)
			SoundtrackState.opponentVocals.time = time;
	}
	...
	public function pauseOrResume(resume:Bool = false) 
	{
		if (resume)
		{
			...
			if (SoundtrackState.vocals != null && SoundtrackState.vocals.length > FlxG.sound.music.time && !SoundtrackState.vocals.playing)
				SoundtrackState.vocals.resume();
			if (SoundtrackState.opponentVocals != null && SoundtrackState.opponentVocals.length > FlxG.sound.music.time && !SoundtrackState.opponentVocals.playing)
				SoundtrackState.opponentVocals.resume();
		}
		else 
		{
			...
			if (SoundtrackState.vocals != null)
				SoundtrackState.vocals.pause();
			if (SoundtrackState.opponentVocals != null)
				SoundtrackState.opponentVocals.pause();
		}
	}
	...
	{
		var length:Int = instance.songs[SoundtrackState.curSelected].songName.length;
	...
	function setPlaybackRate() 
	{
		...
		if (SoundtrackState.vocals != null)
			SoundtrackState.vocals.pitch = playbackRate;
		if (SoundtrackState.opponentVocals != null)
			SoundtrackState.opponentVocals.pitch = playbackRate;
	}
